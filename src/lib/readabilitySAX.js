...
Element.prototype = {
  ...
  getNodes: function () {
    ...
    for (var i = 0; i < nodes.length; i++) {
      if (nodes[i] && nodes[i].nodeType === Node.TEXT_NODE) {
        if (nodes[i].textContent.length > 0) ret.push(nodes[i]);
      } else {
  ...
  toString: function () {
    ...
    for (var i = 0; i < this.children.length; i++) {
      ...
      if (child.nodeType === Node.TEXT_NODE) text.push(child.textContent);
      else text.push(child.toString());
    }
    return text.join("");
  },
...
Readability.prototype._scanLink = function (elem) {
  ...
  if (re_final.test(linkData)) {
    if (!re_nextLink.test(text))
      if (
        !(
          this._scannedLinks[href] &&
          re_nextLink.test(this._scannedLinks[href].text)
        )
      )
        score -= 65;
  }
...
Readability.prototype.onclosetag = function (tagName) {
  ...
  if (tagName in embeds) {
    ...
    if (contentLength === 0) {
      ...
      if (
        elem.children.length === 1 &&
        elem.children[0].nodeType === Node.TEXT_NODE
      )
        return;
    }
    if (elem.info.tagCount.li - 100 > p && tagName !== "ul" && tagName !== "ol")
      return;
    if (
      contentLength < 25 &&
  ...
  filterEmpty: if (
    (tagName in removeIfEmpty ||
      (!this._settings.cleanConditionally && tagName in cleanConditionally)) &&
  ...
  if (
    ...
    var addScore =
      ...
      elem.info.commas +
      Math.min(
        Math.floor((elem.info.textLength + elem.info.linkLength) / 100),
        3,
      );
...
var getCandidateSiblings = function (candidate) {
  ...
  for (var i = 0; i < childNum; i++) {
    ...
    if (childs[i] === candidate);
      ...
      if (childs[i].info.textLength >= 80 && childs[i].info.density < 0.25);
      else if (
...
Readability.prototype.saxParser = function (elem) {
  ...
  function parse(node) {
    ...
    for (var i = 0; i < num; i++) {
      ...
      if (nodeType === Node.TEXT_NODE) me.ontext(childs[i]);
...
Readability.prototype.getArticle = function (useCache) {
  ...
  if (!useCache || this._article === null) {
    ...
    this._article = {
      ...
      getText: function () {
        if (text === null) text = me.getText(elem);
        return text;
      },
      getNodes: function () {
        if (nodes === null) nodes = me.getNodes(elem);
        return nodes;
      },
      getHTML: function () {
        if (html === null) html = me.getHTML(elem);
        return html;
      },
    };
  }
